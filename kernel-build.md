...
# Building Debian 12 kernel (6.12.22 from backports)

```bash

sudo apt update
apt-cache search linux-image | grep -v -e "-rt-" -e "-cloud-"
...
git clone -b debian/6.12/bookworm-backports --single-branch https://salsa.debian.org/kernel-team/linux.git
